...
function doPost(e) {
  try {
    ...
    try {
      ...
      zabelezi("mejl vlasnicima", g1);
    }
    if (PODESAVANJA.POTVRDA_GOSTU) {
      try {
        mejlGostu(u);
      } catch (g2) {
        zabelezi("mejl gostu", g2);
      }
    }
...
function odgovor(ok, poruka) {
  ...
  return ContentService.createTextOutput(JSON.stringify(telo)).setMimeType(
...
function proveri(u) {
  var g = [];
  if (!u.puno_ime || String(u.puno_ime).trim().length < 3)
    g.push("Nedostaje ime i prezime.");
  if (
    !u.email ||
    !/^[^\s@]+@[^\s@]+\.[a-zA-Z]{2,}$/.test(String(u.email).trim())
  )
    g.push("Adresa e-pošte nije ispravna.");
  if (!u.telefon || String(u.telefon).replace(/[^\d+]/g, "").length < 8)
    g.push("Broj telefona nije ispravan.");
  if (!u.dolazak || !u.odlazak) g.push("Nedostaju datumi boravka.");
  if (u.dolazak && u.odlazak && new Date(u.odlazak) <= new Date(u.dolazak))
    g.push("Datum odlaska mora biti posle datuma dolaska.");
...
function previseUpita(list, email) {
  ...
  for (var i = 0; i < opseg.length; i++) {
    ...
    if (
      vreme &&
      new Date(vreme).getTime() > granica &&
      String(opseg[i][5]).toLowerCase() === email.toLowerCase()
    )
      broj++;
  }
...
function uzmiList() {
  var tabela =
    PODESAVANJA.ID_TABELE && PODESAVANJA.ID_TABELE !== "ID_TABELE_OVDE"
...
function pripremiTabelu() {
  ...
  var pravilo = SpreadsheetApp.newDataValidation()
    .requireValueInList(STATUSI, true)
  ...
  [
    140, 110, 110, 120, 130, 210, 170, 105, 105, 80, 75, 65, 110, 110, 320, 150,
    120, 130, 260,
  ].forEach(function (s, i) {
    list.setColumnWidth(i + 1, s);
  });

  list.setFrozenRows(1);
  list
    .getRange(1, 1, 1, KOLONE.length)
    .setFontWeight("bold")
    .setBackground("#1A1714")
    .setFontColor("#FAF7F2");
  SpreadsheetApp.getActiveSpreadsheet().toast(
    "Tabela je pripremljena.",
    "Tontić Lux",
    5,
  );
}
...
function mejlVlasnicima(u) {
  var naslov =
    "[Novi upit] " +
    (u.apartman || "Tontić Lux") +
    ", " +
    u.puno_ime +
    ", " +
    (u.dolazak_prikaz || u.dolazak);
  var redovi = [
    ["Apartman", u.apartman],
  ...
  var html =
    ...
    '<h2 style="font-size:22px;font-weight:500;margin:0 0 18px">' +
    izlaz(u.puno_ime) +
    ", " +
    izlaz(u.nocenja) +
    " noćenja</h2>" +
    ...
    redovi
      .map(function (r) {
        return (
          '<tr><td style="padding:8px 10px 8px 0;color:#6f675c;border-bottom:1px solid #eee;width:38%">' +
          izlaz(r[0]) +
    ...
    izlaz(u.telefon) +
    '" style="background:#8B6F47;color:#fff;padding:10px 18px;border-radius:40px;text-decoration:none;font-size:14px">Pozovi gosta</a>' +
    ' &nbsp; <a href="https://wa.me/' +
    String(u.telefon || "").replace(/[^\d]/g, "") +
    '" style="color:#8B6F47;font-size:14px">WhatsApp</a></p>' +
...
function mejlGostu(u) {
  ...
  var html =
    ...
    izlaz(ime) +
    ",</p>" +
    "<p>primili smo vaš upit za <b>" +
    izlaz(u.apartman) +
    "</b>, " +
    izlaz(u.dolazak_prikaz || u.dolazak) +
    " do " +
    izlaz(u.odlazak_prikaz || u.odlazak) +
    ", za " +
    izlaz(u.gosti) +
    " " +
    (Number(u.gosti) === 1 ? "osobu" : "osoba") +
    ".</p>" +
    ...
    izlaz(PODESAVANJA.TELEFON) +
    ".</p>" +
    '<p style="margin-top:24px">' +
    izlaz(PODESAVANJA.POTPIS) +
    "</p>" +
    "</div>";
  ...
  MailApp.sendEmail({
    ...
    body:
      "Poštovani " +
      ime +
      ", primili smo vaš upit. Javljamo se za 1 do 4 sata. " +
      PODESAVANJA.POTPIS,
  ...
  for (var i = 0; i < podaci.length; i++) {
    ...
    if (odlazak) {
      ...
      if (proslo === 2 && interno.indexOf("[utisak]") === -1) {
        ...
        list
          .getRange(i + 2, 19)
          .setValue((list.getRange(i + 2, 19).getValue() + " [utisak]").trim());
      }
    }
  }
}
  ...
  try {
    console.error(
      "Tontić Lux, " +
        gde +
        ": " +
        (greska && greska.message ? greska.message : greska),
    );
  } catch (e) {
    /* ignorisi */
  }
...
function testUpit() {
  var probni = {
    ime: "Probni",
    prezime: "Gost",
    puno_ime: "Probni Gost",
    email: PODESAVANJA.PRIMA,
    telefon: "0641234567",
    apartman: "TonticLux 53",
    dolazak: "2026-12-20",
    odlazak: "2026-12-23",
    dolazak_prikaz: "20.12.2026.",
    odlazak_prikaz: "23.12.2026.",
    nocenja: 3,
    odrasli: 2,
    deca: 1,
    uzrasti: "6",
